...
impl Database {
    ...
    pub async fn health_check(&self) -> Result<(), sqlx::Error> {
        ...
        // Optional schema guardrails: ensure critical columns exist
        let _ = sqlx::query("SELECT full_name FROM users LIMIT 1")
            .fetch_optional(&*self.pool)
            .await?;
        let _ = sqlx::query("SELECT password_hash FROM users LIMIT 1")
            .fetch_optional(&*self.pool)
            .await?;
        Ok(())
    }
