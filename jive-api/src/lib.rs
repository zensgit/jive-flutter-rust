...
impl AppMetrics {
    ...
    pub fn get_rehash_fail(&self) -> u64 { self.rehash_fail_count.load(Ordering::Relaxed) }

    pub fn inc_export_request_stream(&self) { self.export_request_stream_count.fetch_add(1, Ordering::Relaxed); }
    pub fn inc_export_request_buffered(&self) { self.export_request_buffered_count.fetch_add(1, Ordering::Relaxed); }
    pub fn add_export_rows_stream(&self, n: u64) { self.export_rows_stream.fetch_add(n, Ordering::Relaxed); }
    pub fn add_export_rows_buffered(&self, n: u64) { self.export_rows_buffered.fetch_add(n, Ordering::Relaxed); }
    ...
    pub fn increment_login_fail(&self) { self.auth_login_fail_count.fetch_add(1, Ordering::Relaxed); }
    pub fn increment_login_inactive(&self) { self.auth_login_inactive_count.fetch_add(1, Ordering::Relaxed); }
    pub fn get_login_fail(&self) -> u64 { self.auth_login_fail_count.load(Ordering::Relaxed) }
    pub fn get_login_inactive(&self) -> u64 { self.auth_login_inactive_count.load(Ordering::Relaxed) }
    ...
    fn observe_histogram(
        ...
        if dur_secs <= 0.05 { b005.fetch_add(1, Ordering::Relaxed); }
        if dur_secs <= 0.2 { b02.fetch_add(1, Ordering::Relaxed); }
        if dur_secs <= 1.0 { b1.fetch_add(1, Ordering::Relaxed); }
        if dur_secs <= 3.0 { b3.fetch_add(1, Ordering::Relaxed); }
        if dur_secs <= 10.0 { b10.fetch_add(1, Ordering::Relaxed); }
    ...
    pub fn inc_rehash_fail_hash(&self) { self.rehash_fail_hash.fetch_add(1, Ordering::Relaxed); }
    pub fn inc_rehash_fail_update(&self) { self.rehash_fail_update.fetch_add(1, Ordering::Relaxed); }
