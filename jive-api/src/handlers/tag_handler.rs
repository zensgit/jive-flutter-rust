...
pub async fn list_tags(
    ...
    let family_id = claims
        .family_id
        .ok_or(ApiError::BadRequest("No family selected".into()))?;
...
pub async fn create_tag(
    ...
    if body.name.trim().is_empty() {
        return Err(ApiError::ValidationError("Empty tag name".into()));
    }
    let service = TagService::new(pool);
    let tag = service
        .create_tag(family_id, &body.name, body.color.as_deref(), None)
        .await
...
pub async fn update_tag(
    ...
    let service = TagService::new(pool);
    let tag = service
        .update_tag(id, body.name.as_deref(), body.color.as_deref(), None)
        .await
        .map_err(|e| ApiError::BadRequest(format!("Failed to update tag: {:?}", e)))?;
    Ok(Json(ApiResponse::success(serde_json::json!({"tag": tag}))))
}
...
pub async fn delete_tag(
    ...
    service
        .delete_tag(id)
        .await
        .map_err(|e| ApiError::BadRequest(format!("Failed to delete tag: {:?}", e)))?;
...
pub async fn merge_tags(
    ...
    let service = TagService::new(pool);
    let merged = service
        .merge_tags(family_id, body.from_ids, body.to_id)
        .await
        .map_err(|e| ApiError::BadRequest(format!("Failed to merge tags: {:?}", e)))?;
