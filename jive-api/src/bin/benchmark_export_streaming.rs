...
async fn main() -> anyhow::Result<()> {
    ...
    if db_url.is_empty() {
        eprintln!("Set --database-url or DATABASE_URL");
        std::process::exit(1);
    }
...
async fn seed(pool: &PgPool, rows: i64) -> anyhow::Result<()> {
    ...
    let ledger_row: Option<(uuid::Uuid, uuid::Uuid)> =
        sqlx::query_as("SELECT id, created_by FROM ledgers LIMIT 1")
            .fetch_optional(pool)
            .await?;
    let (ledger_id, user_id) = if let Some(l) = ledger_row {
        l
    } else {
    ...
    let account_id: (uuid::Uuid,) =
        sqlx::query_as("SELECT id FROM accounts WHERE ledger_id=$1 LIMIT 1")
            .bind(ledger_id)
            .fetch_one(pool)
            .await?;
