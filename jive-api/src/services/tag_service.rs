...
#[derive(Debug, Clone, serde::Serialize)]
pub struct TagSummary { pub id: Uuid, pub name: String, pub usage_count: i64 }

pub struct TagService { pool: PgPool }
...
impl TagService {
    ...
    async fn pick_ledger_for_family(&self, family_id: Uuid) -> Result<Uuid, ServiceError> {
        ...
        if let Some(id) = sqlx::query_scalar!("SELECT id FROM ledgers WHERE family_id=$1 AND is_default=true LIMIT 1", family_id)
            .fetch_optional(&self.pool).await? { return Ok(id); }
        let id = sqlx::query_scalar!("SELECT id FROM ledgers WHERE family_id=$1 ORDER BY updated_at DESC LIMIT 1", family_id)
    ...
    pub async fn list_tags(&self, family_id: Uuid, q: Option<String>) -> Result<Vec<TagDto>, ServiceError> {
        ...
        Ok(rows.into_iter().map(|r| TagDto{
            id: r.get("id"),
    ...
    pub async fn create_tag(&self, family_id: Uuid, name: &str, color: Option<&str>, description: Option<&str>) -> Result<TagDto, ServiceError> {
        ...
        let row = sqlx::query!(
            r#"INSERT INTO tags (id, ledger_id, name, color, description, usage_count, created_at)
               ...
               id, ledger_id, name, color, description
        ).fetch_one(&self.pool).await?;
        Ok(TagDto{ id: row.id, ledger_id: row.ledger_id, name: row.name, color: row.color, description: row.description, usage_count: row.usage_count.unwrap_or(0) })
    }

    pub async fn update_tag(&self, id: Uuid, name: Option<&str>, color: Option<&str>, description: Option<&str>) -> Result<TagDto, ServiceError> {
        let row = sqlx::query!(
            r#"UPDATE tags SET
    ...
    pub async fn delete_tag(&self, id: Uuid) -> Result<(), ServiceError> {
        sqlx::query!("DELETE FROM tags WHERE id = $1", id).execute(&self.pool).await?;
        Ok(())
    }

    pub async fn merge_tags(&self, family_id: Uuid, from_ids: Vec<Uuid>, to_id: Uuid) -> Result<i64, ServiceError> {
        let mut tx = self.pool.begin().await?;
        let to_name: String = sqlx::query_scalar!("SELECT name FROM tags WHERE id = $1", to_id).fetch_one(&mut *tx).await?;
        let from_names: Vec<String> = sqlx::query!("SELECT name FROM tags WHERE id = ANY($1)", &from_ids)
        ...
        if !from_names.is_empty() {
            let _ = sqlx::query!(
                ...
                &from_names, &vec![to_name.clone()], family_id
            ).execute(&mut *tx).await?;
        }
        let res = sqlx::query!("DELETE FROM tags WHERE id = ANY($1) AND id <> $2", &from_ids, to_id)
            .execute(&mut *tx).await?;
        tx.commit().await?;
    ...
    pub async fn summary(&self, family_id: Uuid) -> Result<Vec<TagSummary>, ServiceError> {
        ...
        Ok(rows.into_iter().map(|r| TagSummary { id: r.id, name: r.name, usage_count: r.usage_count.unwrap_or(0) as i64 }).collect())
    }
}
