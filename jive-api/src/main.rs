...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = Router::new()
        ...
        .route(
            "/api/v1/auth/user",
            get(auth_handlers::get_current_user).put(auth_handlers::update_user),
        )
        ...
        // 家庭管理 API
        .route("/api/v1/families", get(list_families).post(create_family))
        ...
        .route(
            ...
            get(get_family_members).post(add_member),
        )
        .route(
        ...
        .route("/api/v1/ledgers", get(list_ledgers).post(create_ledger))
        .route("/api/v1/ledgers/current", get(get_current_ledger))
        .route(
            "/api/v1/ledgers/:id",
            get(get_ledger).put(update_ledger).delete(delete_ledger),
        )
        ...
        .route(
            "/api/v1/currencies/global-market-stats",
