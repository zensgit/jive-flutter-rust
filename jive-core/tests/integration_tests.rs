...
async fn test_complete_user_workflow() {
    ...
    println!(
        "✅ 用户登录成功，令牌: {}",
        &login_response.access_token[..20]
    );

    // 4. 验证访问令牌
    let verified_user = auth_service
        ._verify_token(login_response.access_token.clone())
        .await;
...
async fn test_complete_ledger_workflow() {
    ...
    // 1. 创建账本
    let create_request =
        CreateLedgerRequest::new("Integration Test Ledger".to_string(), "USD".to_string());

    let ledger = ledger_service
        ._create_ledger(create_request, context.clone())
        .await;
    ...
    // 2. 获取账本详情
    let retrieved_ledger = ledger_service
        ._get_ledger(ledger.id(), context.clone())
        .await;
    ...
    let updated_ledger = ledger_service
        ._update_ledger(ledger.id(), update_request, context.clone())
        .await;
...
async fn test_complete_account_workflow() {
    ...
    let context =
        ServiceContext::new("test-user-123".to_string()).with_ledger("test-ledger-123".to_string());
    ...
    let account = account_service
        ._create_account(create_request, context.clone())
        .await;
    ...
    let updated_account = account_service
        ._update_balance(account.id(), "1000.00".to_string(), context.clone())
        .await;
    ...
    let accounts = account_service
        ._search_accounts(filter, pagination, context)
        .await;
...
async fn test_complete_transaction_workflow() {
    ...
    let transaction = transaction_service
        ._create_transaction(create_request, context.clone())
        .await;
    ...
    // 2. 添加标签
    let tagged_transaction = transaction_service
        ._add_tags(
            transaction.id(),
    ...
    let transactions = transaction_service
        ._search_transactions(filter, PaginationParams::new(1, 10), context)
        .await;
...
async fn test_complete_category_workflow() {
    ...
    let parent_category = category_service
        ._create_category(parent_request, context.clone())
        .await;
    ...
    let child_category = category_service
        ._create_category(child_request, context.clone())
        .await;
    ...
    // 3. 获取分类树
    let category_tree = category_service
        ._get_category_tree(None, context.clone())
        .await;
    ...
    // 4. 建议分类
    let suggestions = category_service
...
async fn test_service_error_handling() {
    ...
    let result = user_service
        ._create_user(invalid_request, context.clone())
        .await;
...
async fn test_service_context_usage() {
    ...
    let permission_check = auth_service
        ._check_permission(
            "user-123".to_string(),
            "accounts".to_string(),
            "read".to_string(),
...
async fn test_business_logic_validation() {
    ...
    // 1. 测试账本权限
    let permission = ledger_service
        ._check_permission("ledger-123".to_string(), context.clone())
        .await;
    ...
    // 测试普通用户权限
    let user_permission = auth_service
        ._check_permission(
            "user-123".to_string(),
            "accounts".to_string(),
            "read".to_string(),
        ...
        .await;
    assert!(
        user_permission.is_ok() && user_permission.unwrap(),
        "普通用户应该能读取账户"
    );

    // 测试管理功能权限
    let admin_permission = auth_service
        ._check_permission(
            "user-123".to_string(),
            "users".to_string(),
            "delete".to_string(),
...
async fn test_import_service_workflow() {
    ...
    // 1. 预览 CSV 导入
    let csv_data = "Date,Description,Amount,Category\n2024-01-01,Test Transaction,-50.00,Food"
        .as_bytes()
        .to_vec();
    let preview = import_service
        .preview_import(csv_data.clone(), ImportFormat::CSV, context.clone())
        .await;
    ...
    let task = import_service
        .start_import(csv_data, config, mappings, context.clone())
        .await;
    assert!(task.success);
...
async fn test_export_service_workflow() {
    ...
    let task = export_service
        .create_export_task("Test Export".to_string(), options.clone(), context.clone())
        .await;
    assert!(task.success);
    assert!(task.data.is_some());
    println!("✅ 导出任务创建成功");
    ...
    let csv_export = export_service
        .export_to_csv(options.clone(), csv_config, context.clone())
        .await;
    ...
    // 3. 导出到 JSON
    let json_export = export_service
        .export_to_json(options, context.clone())
        .await;
...
async fn test_report_service_workflow() {
    ...
    let income_statement = report_service
        .generate_income_statement(date_from, date_to, context.clone())
        .await;
    ...
    let balance_sheet = report_service
        .generate_balance_sheet(date_to, context.clone())
        .await;
    ...
    // 3. 生成现金流量表
    let cash_flow = report_service
        .generate_cash_flow(date_from, date_to, context.clone())
        .await;
    ...
    // 4. 生成分类分析
    let category_analysis = report_service
        .generate_category_analysis(date_from, date_to, context.clone())
        .await;
    ...
    // 5. 生成趋势分析
    let trend_analysis = report_service
        .generate_trend_analysis(12, ReportPeriod::Monthly, context.clone())
        .await;
...
async fn test_budget_service_workflow() {
    ...
    let budget = budget_service
        .create_budget(create_request, context.clone())
        .await;
    ...
    if let Some(budget_data) = budget.data {
        let progress = budget_service
            .get_budget_progress(budget_data.id.clone(), context.clone())
            .await;
        ...
        // 3. 获取预算历史
        let history = budget_service
            .get_budget_history(budget_data.id.clone(), context.clone())
            .await;
        assert!(history.success);
        assert!(history.data.is_some());
        println!("✅ 预算历史获取成功");
    ...
    // 4. 获取预算建议
    let suggestions = budget_service
        .get_budget_suggestions(BudgetType::Monthly, context.clone())
        .await;
    ...
    // 6. 自动分配预算
    let auto_allocate = budget_service
...
async fn test_scheduled_transaction_service_workflow() {
    ...
    let scheduled = scheduled_service
        .create_scheduled_transaction(create_request, context.clone())
        .await;
    ...
    if let Some(scheduled_data) = scheduled.data {
        ...
        let detail = scheduled_service
            .get_scheduled_transaction(scheduled_data.id.clone(), context.clone())
            .await;
        ...
        // 3. 执行定期交易
        let execution = scheduled_service
            .execute_scheduled_transaction(scheduled_data.id.clone(), context.clone())
            .await;
        ...
        // 4. 暂停定期交易
        let paused = scheduled_service
            .pause_scheduled_transaction(scheduled_data.id.clone(), context.clone())
            .await;
        assert!(paused.success);
        println!("✅ 定期交易暂停成功");

        // 5. 恢复定期交易
        let resumed = scheduled_service
            .resume_scheduled_transaction(scheduled_data.id.clone(), context.clone())
            .await;
        assert!(resumed.success);
        println!("✅ 定期交易恢复成功");

        // 6. 获取执行历史
        let history = scheduled_service
            .get_execution_history(scheduled_data.id.clone(), 10, context.clone())
            .await;
        assert!(history.success);
        println!("✅ 执行历史获取成功");
    }
    ...
    // 7. 获取即将到期的交易
    let upcoming = scheduled_service
        .get_upcoming_transactions(7, context.clone())
        .await;
    assert!(upcoming.success);
    println!("✅ 即将到期交易获取成功");

    // 8. 获取统计信息
    let stats = scheduled_service
        .get_scheduled_statistics(context.clone())
        .await;
...
async fn test_rule_service_workflow() {
    ...
    let rule = rule_service
        .create_rule(create_request, context.clone())
        .await;
    assert!(rule.success);
    ...
    if let Some(rule_data) = rule.data {
        // 2. 获取规则详情
        let detail = rule_service
            .get_rule(rule_data.id.clone(), context.clone())
            .await;
        assert!(detail.success);
        assert!(detail.data.is_some());
        println!("✅ 规则详情获取成功");
        ...
        let test_result = rule_service
            .test_rule(rule_data.id.clone(), test_target.clone(), context.clone())
            .await;
        ...
        // 4. 执行规则
        let execution = rule_service
            .execute_rule(rule_data.id.clone(), test_target, context.clone())
            .await;
        assert!(execution.success);
        assert!(execution.data.is_some());
        ...
        // 5. 获取执行历史
        let history = rule_service
            .get_execution_history(Some(rule_data.id.clone()), 10, context.clone())
            .await;
        assert!(history.success);
        println!("✅ 执行历史获取成功");

        // 6. 获取规则统计
        let stats = rule_service
            .get_rule_statistics(rule_data.id.clone(), context.clone())
            .await;
        assert!(stats.success);
        println!("✅ 规则统计获取成功");
    }
    ...
    let batch_execution = rule_service
        .execute_rules(batch_target, context.clone())
        .await;
    assert!(batch_execution.success);
    println!("✅ 批量规则执行成功");
...
async fn test_tag_service_workflow() {
    ...
    let tag = tag_service
        .create_tag(create_request, context.clone())
        .await;
    assert!(tag.success);
    ...
    if let Some(tag_data) = tag.data {
        // 2. 获取标签详情
        let detail = tag_service
            .get_tag(tag_data.id.clone(), context.clone())
            .await;
        assert!(detail.success);
        assert!(detail.data.is_some());
        println!("✅ 标签详情获取成功");
        ...
        // 5. 获取标签统计
        let stats = tag_service
            .get_tag_statistics(tag_data.id.clone(), context.clone())
            .await;
        assert!(stats.success);
        println!("✅ 标签统计获取成功");
    ...
    let group = tag_service
        .create_tag_group(group_request, context.clone())
        .await;
    ...
    // 9. 搜索标签
    let search_results = tag_service
        .search_tags("Import".to_string(), 10, context.clone())
        .await;
...
async fn test_payee_service_workflow() {
    ...
    let payee = payee_service
        .create_payee(create_request, &context)
        .await
        .unwrap();
    ...
    // 5. 搜索收款方
    let search_results = payee_service
        .search_payees("星", 10, &context)
        .await
        .unwrap();
    ...
    // 7. 获取收款方统计
    let stats = payee_service
        .get_payee_stats(&payee.id, &context)
        .await
        .unwrap();
    ...
    // 8. 获取收款方建议
    let suggestions = payee_service
        .suggest_payees("星巴克咖啡购买", 5, &context)
        .await
        .unwrap();
    ...
    println!(
        "✅ 收款方建议获取成功，置信度: {:.2}",
        suggestions[0].confidence_score
    );
    ...
    let pagination = PaginationParams::new(1, 10);
    let filtered_payees = payee_service
        .get_payees(Some(filter), pagination, &context)
        .await
        .unwrap();
    assert_eq!(filtered_payees.items.len(), 2); // 星巴克和麦当劳
    println!(
        "✅ 带过滤的收款方查询成功，找到 {} 个餐厅类收款方",
        filtered_payees.items.len()
    );

    // 10. 批量更新状态
    let payee_ids = vec![payee.id.clone()];
    let updated_count = payee_service
        .batch_update_status(payee_ids, false, &context)
        .await
        .unwrap();
...
async fn test_notification_service_workflow() {
    ...
    let notification = notification_service
        .create_notification(create_request, &context)
        .await
        .unwrap();
    ...
    // 2. 获取通知详情
    let retrieved_notification = notification_service
    ...
    notification_service
        ...
        .await
        .unwrap();
    let read_notification = notification_service
        .get_notification(&notification.id, &context)
        .await
        .unwrap();
    ...
    for (notification_type, title, message) in notification_types {
        ...
        let created = notification_service
    ...
    let pagination = PaginationParams::new(1, 10);
    let high_priority_notifications = notification_service
    ...
    println!(
        "✅ 高优先级通知查询成功，找到 {} 个通知",
        high_priority_notifications.items.len()
    );
    ...
    let bulk_notification_ids = notification_service
    ...
    println!(
        "✅ 批量通知创建成功，创建 {} 个通知",
        bulk_notification_ids.len()
    );

    // 7. 创建和使用模板
    let template = notification_service
        .create_template(
            "预算警告模板".to_string(),
            NotificationType::BudgetAlert,
    ...
    let template_notification = notification_service
        .create_notification(template_request, &context)
        .await
        .unwrap();
    ...
    // 9. 获取通知统计
    let stats = notification_service
        .get_notification_stats(Some("test-user-notification".to_string()), &context)
        .await
        .unwrap();
    ...
    println!(
        "✅ 通知统计获取成功，总发送: {}，已读率: {:.1}%",
        stats.total_sent, stats.read_rate
    );

    // 10. 批量标记为已读
    let marked_count = notification_service
        .mark_all_as_read("test-user-notification", &context)
        .await
        .unwrap();
    ...
    // 11. 获取模板列表
    let templates = notification_service
        .get_templates(Some(NotificationType::BudgetAlert), &context)
        .await
        .unwrap();
    ...
    notification_service
        .set_user_preferences(preferences, &context)
        .await
        .unwrap();
    println!("✅ 用户通知偏好设置成功");

    // 13. 获取用户通知偏好
    let retrieved_preferences = notification_service
        .get_user_preferences("test-user-notification", &context)
        .await
        .unwrap();
    assert_eq!(retrieved_preferences.user_id, "test-user-notification");
    assert_eq!(retrieved_preferences.enabled_channels.len(), 2);
    assert_eq!(
        retrieved_preferences.quiet_hours_start,
