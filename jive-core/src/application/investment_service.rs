...
impl InvestmentService {
    ...
    pub async fn create_security(
        ...
        if !context.has_permission_str("manage_investments") {
            return Err(JiveError::Forbidden(
                "No permission to manage investments".into(),
            ));
        }

        // 检查证券是否已存在
        if self
            .security_exists(&request.ticker, request.exchange.as_deref())
            .await?
        {
            return Err(JiveError::AlreadyExists(format!(
    ...
    pub async fn execute_trade(
        ...
        if !context.has_permission_str("execute_trades") {
            return Err(JiveError::Forbidden(
                "No permission to execute trades".into(),
            ));
        }

        // 获取账户和证券
        let mut account = self
            .get_investment_account(&context.family_id, &request.account_id)
            .await?;
        ...
        if request.trade_type == TradeType::Buy && total_amount > account.cash_balance {
            return Err(JiveError::ValidationError(
                "Insufficient cash balance".into(),
            ));
        }
        ...
        // 更新账户余额和持仓
        self.update_account_after_trade(&mut account, &trade, &security)
            .await?;
    ...
    pub async fn update_security_price(
        ...
        if !context.has_permission_str("update_prices") {
            return Err(JiveError::Forbidden(
                "No permission to update prices".into(),
            ));
        }
        ...
        // 更新所有持有该证券的账户价值
        self.update_accounts_with_security(&security_id, price)
            .await?;
    ...
    pub async fn get_holdings(
        ...
        if !context.has_permission_str("view_investments") {
            return Err(JiveError::Forbidden(
                "No permission to view investments".into(),
            ));
        }

        let account = self
            .get_investment_account(&context.family_id, &account_id)
            .await?;
    ...
    pub async fn analyze_portfolio(
        ...
        let account = self
            ...
            .await?;
        let holdings = self
            .get_holdings(context.clone(), account_id.clone())
            .await?
            .data
            .unwrap();
        ...
        let convert_to_percentage = |allocation: HashMap<String, Decimal>| -> Vec<AllocationItem> {
            let mut items: Vec<AllocationItem> = allocation
                .into_iter()
        ...
        let performance_metrics = self
            .calculate_performance_metrics(&account, &holdings)
            .await?;
    ...
    pub async fn calculate_capital_gains_tax(
        ...
        let trades = self
            .get_trades_for_tax_year(&context.family_id, &request.account_id, request.tax_year)
            .await?;
        ...
        // 估算税额（简化计算）
        let estimated_tax =
            self.estimate_capital_gains_tax(net_short_term, net_long_term, request.tax_rate)?;
    ...
    pub async fn track_dividend(
        ...
        account.cash_balance +=
            dividend.total_amount - dividend.tax_withheld.unwrap_or(Decimal::ZERO);
    ...
    async fn get_investment_account(
        &self,
        family_id: &str,
        account_id: &str,
            ...
            TradeType::Sell => {
                    ...
                    .find(|h| h.security_id == trade.security_id)
                {
                    ...
                    if holding.quantity <= Decimal::ZERO {
                        account
                            .holdings
                            .retain(|h| h.security_id != trade.security_id);
                    }
                }
            }
        }
    ...
    fn get_top_performers(&self, holdings: &[HoldingDetail], limit: usize) -> Vec<PerformerInfo> {
        let mut performers: Vec<_> = holdings
            .iter()
            .map(|h| PerformerInfo {
