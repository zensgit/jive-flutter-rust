...
use crate::domain::{Transaction, TransactionType, Category, Account, Tag, Payee};
use crate::error::{JiveError, Result};
use crate::application::{ServiceContext, ServiceResponse, BatchResult};
...
impl DataExchangeService {
    ...
    pub async fn export_transactions(
        ...
        // 获取数据
        let transactions = self.get_transactions_for_export(
            &context.family_id,
            &request.filters,
        ).await?;
        ...
        // 记录导出日志
        self.log_export(&context, &filename, transactions.len()).await?;
    ...
    pub async fn export_accounts(
        ...
        let file_content = match request.format {
            ...
            _ => return Err(JiveError::ValidationError("Unsupported format for accounts".into())),
        };
    ...
    pub async fn export_full_backup(
        ...
        if !context.has_permission_str("manage_family") {
            return Err(JiveError::Forbidden("No permission to create backup".into()));
        }
    ...
    pub async fn import_transactions(
        ...
        for row in parsed_rows {
            match self.import_single_transaction(&context, &row, &mapping).await {
                Ok(transaction) => {
        ...
        if request.apply_rules {
            self.apply_rules_to_transactions(&context, &imported_transactions).await?;
        }
    ...
    pub async fn restore_backup(
        ...
        // 权限检查 - 需要最高权限
        if !context.has_permission_str("manage_family") {
            return Err(JiveError::Forbidden("No permission to restore backup".into()));
        }
        ...
        if let Some(expected_checksum) = &request.checksum {
            ...
            if actual_checksum != *expected_checksum {
                return Err(JiveError::ValidationError("Backup checksum mismatch".into()));
            }
        }
        ...
        if !self.is_compatible_version(&backup_data.version) {
            return Err(JiveError::ValidationError(
                format!("Incompatible backup version: {}", backup_data.version)
            ));
        }
        ...
        // 恢复顺序很重要，先恢复基础数据
        restore_stats.accounts = self.restore_accounts(&context, &backup_data.accounts).await?;
        restore_stats.categories = self.restore_categories(&context, &backup_data.categories).await?;
        ...
        // 然后恢复交易数据
        restore_stats.transactions = self.restore_transactions(&context, &backup_data.transactions).await?;
    ...
    pub async fn preview_import(
        ...
        let parsed_rows = match request.format {
            ...
            _ => return Err(JiveError::NotImplemented("Preview only supports CSV".into())),
        };
        ...
        // 生成智能映射建议
        let mapping_suggestions = self.generate_mapping_suggestions(&context, &parsed_rows).await?;
    ...
    fn export_to_csv(&self, transactions: &[TransactionExport]) -> Result<Vec<u8>> {
        ...
        // 写入表头
        wtr.write_record(&[
            "Date", "Amount", "Type", "Category", "Payee", 
            "Account", "Description", "Tags", "Notes"
        ])?;
    ...
    fn export_to_pdf(&self, transactions: &[TransactionExport], options: &ExportOptions) -> Result<Vec<u8>> {
    ...
    fn parse_csv_record(&self, record: &StringRecord) -> Result<ImportRow> {
        Ok(ImportRow {
            date: record.get(0).and_then(|s| NaiveDate::parse_from_str(s, "%Y-%m-%d").ok()),
            ...
            tags: record.get(6).map(|s| s.split(',').map(String::from).collect()).unwrap_or_default(),
    ...
    fn parse_json(&self, content: &[u8]) -> Result<Vec<ImportRow>> {
        ...
        Ok(transactions.into_iter().map(|t| ImportRow {
            date: Some(t.date),
    ...
    fn parse_mint_csv(&self, content: &[u8]) -> Result<Vec<ImportRow>> {
        ...
        for result in rdr.records() {
            ...
            rows.push(ImportRow {
                date: record.get(0).and_then(|s| NaiveDate::parse_from_str(s, "%m/%d/%Y").ok()),
                ...
                tags: record.get(7).map(|s| s.split(',').map(String::from).collect()).unwrap_or_default(),
        ...
        for row in rows {
            if let Some(cat_name) = &row.category {
                if !mapping.category_map.contains_key(cat_name) {
                    // 查找匹配的分类
                    let matched = categories.iter()
                        .find(|c| c.name.eq_ignore_ascii_case(cat_name))
                        .or_else(|| {
                            // 模糊匹配
                            categories.iter().find(|c| c.name.contains(cat_name) || cat_name.contains(&c.name))
                        });
                    
                    if let Some(category) = matched {
                        mapping.category_map.insert(cat_name.clone(), category.id.clone());
                    }
                }
            }
        }
        ...
        for row in rows {
            if let Some(payee_name) = &row.payee {
                if !mapping.payee_map.contains_key(payee_name) {
                    let matched = payees.iter()
                        .find(|p| p.name.eq_ignore_ascii_case(payee_name));
                    
                    if let Some(payee) = matched {
                        mapping.payee_map.insert(payee_name.clone(), payee.id.clone());
                    }
                }
            }
        }
    ...
    async fn import_single_transaction(
        ...
        let transaction = TransactionData {
            id: Uuid::new_v4().to_string(),
            family_id: context.family_id.clone(),
            date: row.date.ok_or_else(|| JiveError::ValidationError("Missing date".into()))?,
            amount: row.amount.ok_or_else(|| JiveError::ValidationError("Missing amount".into()))?,
            ...
            category_id: row.category.as_ref()
                ...
                .cloned(),
            payee_id: row.payee.as_ref()
                .and_then(|p| mapping.payee_map.get(p))
                .cloned(),
            account_id: row.account.as_ref()
    ...
    async fn generate_mapping_suggestions(
        ...
        for row in rows.iter().take(5) {
            if let Some(desc) = &row.description {
                ...
                } else if desc.to_lowercase().contains("uber") || desc.to_lowercase().contains("lyft") {
    ...
    async fn log_export(&self, context: &ServiceContext, filename: &str, count: usize) -> Result<()> {
        // TODO: 记录导出日志
        Ok(())
    }
    
    async fn restore_accounts(&self, context: &ServiceContext, accounts: &[AccountExport]) -> Result<usize> {
        // TODO: 恢复账户
        Ok(accounts.len())
    }
    
    async fn restore_categories(&self, context: &ServiceContext, categories: &[CategoryExport]) -> Result<usize> {
        // TODO: 恢复分类
        Ok(categories.len())
    }
    ...
    async fn restore_payees(&self, context: &ServiceContext, payees: &[PayeeExport]) -> Result<usize> {
        // TODO: 恢复商户
        Ok(payees.len())
    }
    
    async fn restore_transactions(&self, context: &ServiceContext, transactions: &[TransactionExport]) -> Result<usize> {
        // TODO: 恢复交易
        Ok(transactions.len())
    }
    
    async fn restore_budgets(&self, context: &ServiceContext, budgets: &[BudgetExport]) -> Result<usize> {
        // TODO: 恢复预算
        Ok(budgets.len())
    }
...
mod tests {
    ...
    fn test_import_mapping_summary() {
        let mut mapping = ImportMapping::default();
        mapping.category_map.insert("Food".to_string(), "cat-1".to_string());
        mapping.account_map.insert("Checking".to_string(), "acc-1".to_string());
