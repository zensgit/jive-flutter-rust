...
impl RuleService {
    ...
    pub async fn create_rule(
        ...
        if request.name.is_empty() {
            return ServiceResponse::error(
                JiveError::ValidationError { message: "Rule name is required".to_string() }
            );
        }

        if request.conditions.is_empty() {
            return ServiceResponse::error(
                JiveError::ValidationError { message: "At least one condition is required".to_string() }
            );
        }

        if request.actions.is_empty() {
            return ServiceResponse::error(
                JiveError::ValidationError { message: "At least one action is required".to_string() }
            );
        }
    ...
    pub async fn update_rule(
        ...
        if let Some(rule) = storage.iter_mut().find(|r| r.id == id) {
            ...
            ServiceResponse::error(
                JiveError::NotFound { message: format!("Rule {} not found", id) }
            )
        }
    }
                ...
                if let Some(ref search) = filter.search {
                    if !r.name.to_lowercase().contains(&search.to_lowercase()) &&
                       !r.description.as_ref().map_or(false, |d| 
                           d.to_lowercase().contains(&search.to_lowercase())) {
                        return false;
                    }
                }
                true
    ...
    pub async fn execute_rule(
        ...
        if let Some(rule) = storage.iter().find(|r| r.id == rule_id) {
            ...
            // 检查条件
            let conditions_met = self.evaluate_conditions(&rule.conditions, &rule.condition_logic, &target);
            ...
            ServiceResponse::error(
                JiveError::NotFound { message: format!("Rule {} not found", rule_id) }
            )
        }
    }
    ...
    pub async fn execute_rules(
        ...
        ServiceResponse::success_with_message(
            results,
            format!("Executed rules for target")
        )
    }
    ...
    pub async fn test_rule(
        ...
        if let Some(rule) = storage.iter().find(|r| r.id == rule_id) {
            ...
            let overall_match = self.evaluate_conditions(&rule.conditions, &rule.condition_logic, &test_target);
    ...
    pub async fn create_rule_from_template(
        ...
        if let Some(template) = templates.iter().find(|t| t.id == template_id) {
            ...
            for (key, value) in customization {
                ...
                for condition in &mut conditions {
                    if condition.value.contains(&format!("{{{{{}}}}}", key)) {
                        condition.value = condition.value.replace(&format!("{{{{{}}}}}", key), &value);
                    }
                }
            ...
            ServiceResponse::error(
                JiveError::NotFound { message: format!("Template {} not found", template_id) }
            )
        }
    }
    ...
    pub async fn export_rules(
        ...
        let export_data: Vec<RuleExportData> = rules.into_iter()
    ...
    pub async fn optimize_rule_order(
        ...
        storage.sort_by(|a, b| {
            ...
            // 然后按执行次数
            b.statistics.total_executions.cmp(&a.statistics.total_executions)
        });
    ...
    fn evaluate_conditions(
        ...
        match logic {
            ...
            ConditionLogic::Any => {
                conditions.iter().any(|c| self.evaluate_single_condition(c, target))
            }
    ...
    fn evaluate_single_condition(&self, condition: &RuleCondition, target: &RuleTarget) -> bool {
        ...
        match &condition.operator {
            ...
            ConditionOperator::GreaterThan => {
                if let (Ok(field), Ok(cond)) = (field_value.parse::<f64>(), condition.value.parse::<f64>()) {
                    field > cond
                } else {
                    false
                }
    ...
    fn execute_action(&self, action: &RuleAction, target: &RuleTarget) -> Result<String> {
        match &action.action_type {
            ActionType::SetCategory => {
                let category_id = action.parameters.get("category_id")
            ...
            ActionType::AddTag => {
                let tag = action.parameters.get("tag")
                    .ok_or(JiveError::ValidationError { 
            ...
            ActionType::SetField => {
                let field = action.parameters.get("field")
                    .ok_or(JiveError::ValidationError { 
    ...
    fn preview_action(&self, action: &RuleAction, target: &RuleTarget) -> String {
        match &action.action_type {
            ActionType::SetCategory => {
                format!("Would set category to: {}", 
                    action.parameters.get("category_id").unwrap_or(&"unknown".to_string()))
            }
            ActionType::AddTag => {
                format!("Would add tag: {}", 
                    action.parameters.get("tag").unwrap_or(&"unknown".to_string()))
            }
