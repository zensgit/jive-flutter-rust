...
impl CreditCardService {
    ...
    pub async fn create_credit_card(
        ...
        let credit_card = CreditCard {
            ...
            // 额度管理
            credit_limit_type: request.credit_limit_type.unwrap_or(CreditLimitType::Individual),
            ...
            payment_date_type: request.payment_date_type.unwrap_or(PaymentDateType::FixedDate),
            payment_date: request.payment_date,
            payment_days_after_bill: request.payment_days_after_bill,
            bill_calculation_in_previous_period: request.bill_calculation_in_previous_period.unwrap_or(false),
            grace_period_days: request.grace_period_days.unwrap_or(21),
            
            // 利率和费用
            annual_fee: request.annual_fee.unwrap_or(Decimal::ZERO),
            apr: request.apr.unwrap_or(Decimal::from_str_exact("0.1899").unwrap()),
            cash_advance_apr: request.cash_advance_apr,
            penalty_apr: request.penalty_apr,
            foreign_transaction_fee: request.foreign_transaction_fee.unwrap_or(Decimal::from_str_exact("0.03").unwrap()),
            late_payment_fee: request.late_payment_fee,
            
            // 奖励计划
            rewards_program: request.rewards_program,
            base_rewards_rate: request.base_rewards_rate.unwrap_or(Decimal::from_str_exact("0.01").unwrap()),
            category_rewards: request.category_rewards.unwrap_or_default(),
            rewards_cap: request.rewards_cap,
            
            // 多币种
            supported_currencies: request.supported_currencies.unwrap_or_else(|| vec!["USD".to_string()]),
    ...
    pub async fn calculate_billing_cycle(
        ...
        let (start_date, end_date) = if card.bill_calculation_in_previous_period {
            ...
            if for_date.day() <= card.bill_date {
                ...
                let prev_prev_month = for_date.with_day(1).unwrap().pred_opt().unwrap().pred_opt().unwrap();
                let prev_month = for_date.with_day(1).unwrap().pred_opt().unwrap();
                
                let start = prev_prev_month.with_day(card.bill_date.min(days_in_month(prev_prev_month))).unwrap().succ_opt().unwrap();
                let end = prev_month.with_day(card.bill_date.min(days_in_month(prev_month))).unwrap();
                (start, end)
            } else {
                // 下月账单周期：上月账单日+1 到 当月账单日
                let prev_month = for_date.with_day(1).unwrap().pred_opt().unwrap();
                
                let start = prev_month.with_day(card.bill_date.min(days_in_month(prev_month))).unwrap().succ_opt().unwrap();
                let end = for_date.with_day(card.bill_date.min(days_in_month(for_date))).unwrap();
                (start, end)
            }
        } else {
            // 常规账单周期计算
            if for_date.day() <= card.bill_date {
                ...
                let start = for_date.with_day(card.bill_date.min(days_in_month(for_date))).unwrap().succ_opt().unwrap();
                let end = next_month.with_day(card.bill_date.min(days_in_month(next_month))).unwrap();
                (start, end)
            }
        };
        ...
        match card.payment_date_type {
            PaymentDateType::FixedDate => {
                ...
                if payment_day > bill_date.day() {
                    ...
                    Ok(next_month.with_day(payment_day.min(days_in_month(next_month))).unwrap())
                }
            }
    ...
    /// 计算利息
    fn calculate_interest(&self, card: &CreditCard, transactions: &[CreditCardTransaction]) -> Result<Decimal> {
    ...
    pub async fn process_transaction(
        ...
        let mut card = self.get_credit_card(&context.family_id, &request.card_id).await?;
        ...
        let transaction = match request.transaction_type {
            TransactionType::Purchase => {
                self.process_purchase(&mut card, &request).await?
            }
            TransactionType::Payment => {
                self.process_payment(&mut card, &request).await?
            }
            TransactionType::CashAdvance => {
                self.process_cash_advance(&mut card, &request).await?
            }
            TransactionType::Refund => {
                self.process_refund(&mut card, &request).await?
            }
            _ => {
                return Err(JiveError::ValidationError("Invalid transaction type".into()));
            }
        };
    ...
    async fn process_purchase(
        ...
        if request.amount > card.available_credit {
            return Err(JiveError::ValidationError("Insufficient credit limit".into()));
        }
        ...
        let (amount_in_base, exchange_rate) = if let Some(currency) = &request.currency {
            if currency != &card.supported_currencies[0] {
                // 需要货币转换
                let rate = self.get_exchange_rate(currency, &card.supported_currencies[0]).await?;
        ...
        let transaction = CreditCardTransaction {
            ...
            transaction_date: request.transaction_date.unwrap_or_else(|| Utc::now().date_naive()),
        ...
        if let Some(currency) = &request.currency {
            if currency != &card.supported_currencies[0] && !card.auto_convert_currency {
                let foreign_balance = card.foreign_balances.entry(currency.clone()).or_insert(Decimal::ZERO);
    ...
    async fn process_payment(
        ...
        let transaction = CreditCardTransaction {
            ...
            description: request.description.clone().unwrap_or("Payment received".to_string()),
    ...
    async fn process_cash_advance(
        ...
        if total_amount > card.available_credit {
            return Err(JiveError::ValidationError("Insufficient credit for cash advance".into()));
        }
        ...
        let transaction = CreditCardTransaction {
            ...
            description: request.description.clone().unwrap_or("Cash advance".to_string()),
    ...
    pub async fn manage_shared_limit(
        ...
        // 获取共享组中的所有卡片
        let cards = self.get_cards_in_shared_group(
            &context.family_id,
            &request.shared_limit_group_id,
        ).await?;
        
        // 计算总使用额度
        let total_used: Decimal = cards.iter().map(|c| c.current_balance).sum();
        let total_limit = cards.first()
        ...
        let info = SharedLimitInfo {
            ...
            total_used,
            available,
            cards: cards.into_iter().map(|c| CardSummary {
                card_id: c.id,
                card_name: c.name,
    ...
    pub async fn get_rewards_report(
        ...
        let report = RewardsReport {
            ...
            total_purchases: transactions.iter()
                .filter(|t| t.transaction_type == TransactionType::Purchase)
                .map(|t| t.amount)
                .sum(),
    ...
    pub async fn get_optimization_suggestions(
        ...
        if card.rewards_program.is_some() {
            suggestions.push(OptimizationSuggestion {
                ...
                description: "Use this card for purchases in bonus categories to earn more rewards.".to_string(),
                impact: ImpactLevel::Medium,
                potential_savings: Some(card.total_rewards_earned * Decimal::from_str_exact("0.2").unwrap()),
            });
        }
        ...
        if card.foreign_transaction_fee > Decimal::ZERO {
            suggestions.push(OptimizationSuggestion {
                ...
                description: "Consider a card with no foreign transaction fees for international purchases.".to_string(),
    ...
    async fn get_cards_in_shared_group(&self, family_id: &str, group_id: &str) -> Result<Vec<CreditCard>> {
        // TODO: 获取共享组中的所有卡片
        Ok(Vec::new())
    }
}
...
mod tests {
    ...
    fn test_billing_cycle_calculation() {
        ...
        let due_date = service.calculate_payment_due_date(
            &card,
    ...
    fn test_minimum_payment_calculation() {
        ...
        let min_payment = service.calculate_minimum_payment(dec!(1000), &card).unwrap();
