...
class CategoryProvider extends ChangeNotifier {
  ...
  Future<void> convertCategoryToTag(
    ...
    try {
      ...
      // 如果选择删除分类，从分类列表中移除
      if (deleteCategory) {
        await _ref.read(userCategoriesProvider.notifier).deleteCategory(categoryId);
      }
  ...
  Future<void> duplicateCategory(String categoryId, String newName) async {
    try {
      ...
      await _ref.read(userCategoriesProvider.notifier).createCategory(newCategory);
...
/// 分类管理Provider
final categoryManagementProvider = ChangeNotifierProvider<CategoryProvider>((ref) {
