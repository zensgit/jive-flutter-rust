...
class NetworkCategoryService {
  ...
  Future<List<SystemCategoryTemplate>> getTemplates({
    ...
    try {
      ...
      final response = await _dio.get('/api/v1/templates/list',
        queryParameters: {
          if (language != null) 'lang': language,
          if (classification != null) 'type': _classificationToString(classification),
      ...
      final templates = data.map((json) => 
        SystemCategoryTemplate.fromNetworkJson(json)
      ).toList();
  ...
  Future<void> incrementalSync() async {
    try {
      ...
      // 获取更新
      final response = await _dio.get('/api/v1/templates/updates',
        queryParameters: {
  ...
  Future<void> submitUsageStats(String templateId) async {
    try {
      // 异步提交，不影响用户操作
      _dio.post('/api/v1/templates/usage',
        data: {
  ...
  bool _shouldRefresh() {
    ...
    final cacheDuration = isLoggedIn ? _loggedInCacheDuration : _guestCacheDuration;
  ...
  Future<List<SystemCategoryTemplate>?> _getCachedTemplates() async {
    try {
      ...
      return data.map((item) => 
        SystemCategoryTemplate.fromJson(item)
      ).toList();
    } catch (e) {
  ...
  Future<void> _applyUpdates(List<dynamic> updates) async {
    ...
    for (final update in updates) {
      ...
      switch (action) {
        case 'add':
        case 'update':
          final template = SystemCategoryTemplate.fromNetworkJson(update['template']);
